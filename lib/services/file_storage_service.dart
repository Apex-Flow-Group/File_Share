...
class FileStorageService {
  Future<String?> saveFile(String fileName, List<int> data) async {
    try {
      ...
      ApexLogger.instance.log('STORAGE', '💾 حفظ ${_formatSize(data.length)} إلى: $filePath', LogLevel.warning);
      await file.writeAsBytes(data);
      
      final savedSize = await file.length();
      ApexLogger.instance.log('STORAGE', '✅ تم حفظ الملف: ${_formatSize(savedSize)}', LogLevel.success);
  ...
  Future<List<FileSystemEntity>> getReceivedFiles({int? limit}) async {
    try {
      ...
      ApexLogger.instance.log('STORAGE', '❌ فشل قراءة الملفات: $e', LogLevel.error);
      return [];
    }
  }
